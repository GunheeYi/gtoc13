...
function propagatedArc = Trajectory_flybyTargeting_withSails(conicArc, allow_retrograde, allow_low_pass)
    global AU; %#ok<GVMIS>
    ...
    propagatedArc_coarse = refineArcUsingSail_coarse(conicArc, allow_retrograde, allow_low_pass);
    ...
    propagatedArc = refineArcUsingSail_precise(propagatedArc_coarse, allow_retrograde, allow_low_pass);
    fprintf('  Precise optimization produced dr_res = %.2fkm (%.2fAU)\n', ...
        propagatedArc.dr_res, propagatedArc.dr_res / AU);
end

function propagatedArc = refineArcUsingSail_coarse(conicArc, allow_retrograde, allow_low_pass)
    propagatedArc = PropagatedArc(conicArc.t_start, ...
        conicArc.R_start, conicArc.V_start, conicArc.t_end, conicArc.target);
    
    [propagatedArc, ~] = refineLastControls(propagatedArc, propagatedArc.n_controls, allow_retrograde, allow_low_pass, 1e7);
end

function propagatedArc = refineArcUsingSail_precise(propagatedArc_coarse, allow_retrograde, allow_low_pass)
    n_controls_tail = 2;
    propagatedArc = propagatedArc_coarse.splitControls_tail(n_controls_tail);
    [propagatedArc, flag] = ...
        refineLastControls(propagatedArc, propagatedArc.n_controls_tail, allow_retrograde, allow_low_pass, 1e3);
...
function [propagatedArc, flag] = refineLastControls(propagatedArc, n_controls_last, allow_retrograde, allow_low_pass, FitnessLimit)
    ...
    function dr_res = fun(x)
        ...
        if ~allow_retrograde && ~propagatedArc.isPrograde
            return;
        end
