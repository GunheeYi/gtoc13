function propagatedArc = PropagatedArc_updateLastControlsFromVector(propagatedArc, vector)
    ...
    if mod((n_vector-3), 2) ~= 0 % except three variables: two from flyby geometry and one as `global_dt_scaling_factor`
        error('Input vector length must be even (pairs of alpha, beta).');
    end

    n_controls_last = (n_vector-3) / 2;
    ...
    for i = 1:n_controls_last
        ...
        control.alpha = vector(2 + (2*i-1)); % first two values in vector are flyby geometries
        control.beta = vector(2 + (2*i));
